...
source = [MIN_LOC, MIN_LOC, MIN_HEIGHT, R_MAX]
dest = [MAX_LOC, MAX_LOC, MAX_HEIGHT, R_MAX]
agent2 = [3, 3, 3, 3]
...
class reward_set:
    ...
    def cal_throughput(self, adj_array):
        ...
        # nx.draw(graph)
        # plt.show()
        if nx.has_path(graph, N, N + 1):
            path_hop = nx.shortest_path_length(graph, N, N + 1)
        else:
            path_hop = np.inf
...
class My_PPO(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = gym.spaces.Box(low=np.array([MIN_LOC, MIN_LOC, MIN_LOC, 0]),
                                                high=np.array([MAX_LOC, MAX_LOC, MAX_HEIGHT, R_MAX]),
                                                dtype=int)
    ...
    def step(self, action):
        ...
        if self.done:
            ...
            # action을 모두 수행
            for i in range(4):
                self.next_state[i] += action[i] - 1

            # x,y,z좌표 이동범위, txr 가능범위 넘었나 확인
            for i in range(0, 2, 1):  # x,y좌표 이동범위 넘었나 확인
                if (self.next_state[i] > MAX_LOC) or (self.next_state[i] < MIN_LOC):
                    self.next_state[i] -= (action[i] - 1)
            if (self.next_state[2] > MAX_HEIGHT) or (self.next_state[2] < MIN_HEIGHT):  # z좌표 이동범위 넘었나 확인
                self.next_state[2] -= (action[2] - 1)
            if (self.next_state[3] > R_MAX) or (self.next_state[3] < 0):  # txr 가능범위 넘었나 확인
                self.next_state[3] -= (action[3] - 1)

            state_position_array = np.zeros((N + 2, 4))
            state_position_array[0] = self.next_state
            state_position_array[1] = agent2.copy()
            state_position_array[2] = source.copy()
            state_position_array[3] = dest.copy()

            next_position_array = np.zeros((N + 2, 4))
            next_position_array[0] = self.next_state
            next_position_array[1] = agent2.copy()
            next_position_array[2] = source.copy()
            next_position_array[3] = dest.copy()
            ...
            foot = env.cal_foot_of_perpendicular(state_position_array, next_position_array, source, dest, 0, action[3])
            e_move = env.cal_used_energy_to_move(action)
